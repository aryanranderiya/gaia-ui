...
```tsx
import { ChatConversation } from "@/components/ui/message-bubble";
...
export default function Example() {
  const messages = [
    ...
    },
    {
      id: "2",
      variant: "sent",
      messages: ["I'm great! How about you?"],
      timestamp: "10:31 AM",
    },
  ];

  return <ChatConversation messages={messages} />;
}
```
...
A container for multiple chat messages.

**Props**

| Prop      | Type                                         | Default   | Description            |
| --------- | -------------------------------------------- | --------- | ---------------------- |
| messages  | `Array<{id, variant, messages, timestamp?}>` | -         | Array of messages      |
| className | string                                       | undefined | Additional CSS classes |
